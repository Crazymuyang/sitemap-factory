...
class SitemapFactory{
   ...
   *   @param {String} path 填写文件保存路径，通过 path 处理的【path.resolve(__dirname, '../')】 | 必填
   ...
   *   @param {Number} loopspan 循环时间间隔，设置为大于 20 的整数，默认为 60
  ...
  init(options) {
    ...
    let loopspan = Number(options.loopspan) || 100;
    loopspan = loopspan < 60 ? 60 : loopspan;
    ...
    process.on('exit', () => {
      fs.writeFileSync(`${this.path}/${this.filename}.xml`, this.sitemap(this.siteMapSource));
    });
  }
  ...
  URLLoop() {
    ...
    this.loopInterval = setInterval(() => {
      ...
      // if(this.errorURLRepository.length > 10) {
      //   return this.exit()
      // }
    ...
    try{
      ...
      if($a.length) {
        $a.each((idx, ele) => {
          ...
          isvalid && this.suitableURLStore(url);
        });
      }
      return true;
    }catch(err) {
       ...
       * @todo 对各种异常分情况讨论
       */
      !this.isErr && this.errorURLRepository.push(URL);
      return err;
    }
  }
...
return `
  ...
  terminalLog(isEnd = false) {
    ...
    const status = this.isErr ? '(err)' : '';
    if(!isEnd) {
      log(`\x1B[32m 耗时：${this.costTime} || 收录/总数：${resolveRate} || 错误链接：${errorCount} || 当前${status}：${decodeURI(this.currentURL)}\x1B[39m`);
    }else {
